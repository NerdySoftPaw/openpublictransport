...
class BVGProvider(BaseProvider):
    ...
    async def fetch_departures(
        ...
        """Fetch departures from VBB REST API."""
        if not station_id:
            _LOGGER.warning("BVG provider requires a station_id")
            return None
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                if response.status == 200:
                    ...
                    if not isinstance(data, dict) or "departures" not in data:
                        _LOGGER.warning("BVG API unexpected response format")
                        return {"stopEvents": []}
                    # Map FPTF departures to our stopEvents format
                    return {"stopEvents": data["departures"]}
                else:
                    _LOGGER.warning("BVG API returned status %s", response.status)
        except aiohttp.ClientError as e:
            _LOGGER.warning("BVG API request failed: %s", e)
        except Exception as e:
            _LOGGER.warning("BVG API error: %s", e)
    ...
    def parse_departure(
        ...
        try:
            ...
            _LOGGER.debug("Error parsing BVG departure: %s", e)
            return None

    async def search_stops(self, search_term: str) -> List[Dict[str, Any]]:
        """Search for stops using VBB REST API."""
        ...
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status == 200:
                    ...
                    _LOGGER.error("BVG API returned status %s", response.status)
        except Exception as e:
            _LOGGER.error("Error searching BVG stops: %s", e)
